...
-- ============================================
IF OBJECT_ID('tempdb..#TablesWithData2') IS NOT NULL
    DROP TABLE #TablesWithData2;
...
BEGIN
    SET @SQL = '
        IF EXISTS (
            ...
            WHERE ADDED_DTE >= ''2026-07-22 13:15:00'' 
              AND ADDED_DTE <= ''2026-07-22 13:30:00''
        )
        BEGIN
            INSERT INTO #TablesWithData2 VALUES (''' + @SchemaName + '.' + @TableName + ''');
        END
    ';
...
SELECT 表名 FROM #TablesWithData2 ORDER BY 表名;
